...
export default function BasicModal() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (modalidad === 'full' && selectedRemolqueDelantero === selectedRemolqueTrasero) {
      ...
      if (window.confirm(`Confirmar configuración:\n\n${JSON.stringify(configuracion, null, 2)}\n\n¿Deseas continuar?`)) {
        ...
        try {
          const response = await fetch('http://localhost:3011/api/insertConfiguracion', {
            method: 'POST',
            headers: {
  ...
  const fetchTractocamiones = async () => {
    try {
      const res = await fetch('http://localhost:3011/api/getTractocamionesDisponibles');
      if (!res.ok) {
  ...
  const fetchRemolques = async () => {
    try {
      const res = await fetch('http://localhost:3011/api/getRemolquesDisponibles');
      if (!res.ok) {
        throw new Error('Failed to fetch data');
      }
  ...
  const fetchDollies = async () => {
    try {
      const res = await fetch('http://localhost:3011/api/getDolliesDisponibles');
      if (!res.ok) {
        throw new Error('Failed to fetch data');
      }
